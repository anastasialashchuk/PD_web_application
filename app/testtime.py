...
class VideoProcessingTimer:
    ...
    def get_processing_time_per_video(self):
        if not self.processing_times:
            return 0
        return sum(self.processing_times) / len(self.processing_times)

    def get_processing_time_per_frame(self):
        if self.frame_count == 0 or not self.processing_times:
            return 0
        total_frames = sum([t * TARGET_FPS for t in self.processing_times])  # Примерное количество кадров
        return sum(self.processing_times) / total_frames if total_frames > 0 else 0
...
class HandTracker:
    def __init__(self, source="browser"):
        ...
        self.processing_timer = VideoProcessingTimer()
    ...
    def stop_recording(self):
        ...
        if frames_recorded > 0 and processing_time > 0:
            time_per_frame = processing_time / frames_recorded
            print(
                f"Video processed in {processing_time:.2f} seconds ({frames_recorded} frames, {time_per_frame:.4f} sec/frame)")
    ...
    def process_image(self, img):
        ...
        if self.recording_started and self.video_writer is not None:
            ...
            self.frame_count += 1
            self.processing_timer.update_frame_count()
...
@app.route('/get_processing_stats', methods=['GET'])
def get_processing_stats():
    try:
        stats = {
            'average_time_per_video': tracker.processing_timer.get_processing_time_per_video(),
            'average_time_per_frame': tracker.processing_timer.get_processing_time_per_frame(),
            'total_videos_processed': len(tracker.processing_timer.processing_times),
            'total_frames_processed': tracker.processing_timer.frame_count
        }
